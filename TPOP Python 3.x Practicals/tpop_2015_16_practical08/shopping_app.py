...
class Clothing(Item):
    ...
    def __init__(self, barcode, name, ppu, vat, size, description = '', units = 1):
        super().__init__(barcode, name, ppu, vat, description, units)
        self._size = size
...
class Basket(object):
    ...
    def __init__(self):
        self._content = {} # keys are item barcode, values are tuples (quantity,Item i) of that item
    ...
    def remove_all(self, item):
        if item.get_barcode() in self._content:
            quantity, it = self._content.pop(item.get_barcode())
            return quantity
        else:
            raise ValueError('No such item.')
